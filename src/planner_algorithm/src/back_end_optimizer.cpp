...
int TrajOptimizer::optimize_traj_lmbm(
    const Eigen::Matrix3d &initS, const Eigen::Matrix3d &finalS, const std::vector<Eigen::Vector3d> &Q, const Eigen::VectorXd &T, const int N, Trajectory<TRAJ_ORDER> &traj) {
    total_opt_time = 0.0;
    total_sdf_time = 0.0;
    total_AABB_time = 0.0;
    ...
    int ret = lmbm::lmbm_optimize(total_opt_variable_num, x_variable, &final_cost, costFunctionLmbm, this, earlyexitLmbm, &param);
    {
        printf("-----------\n");
        printf("| Output: |\n");
        printf("-----------\n");
    ...
    if (ret >= 0) {
        ...
        std::cout << "[Optimization LMBM] Optimization Success. Final Cost = " << final_cost << std::endl;
        if (x_variable != NULL) {
            free(x_variable);
            x_variable = NULL;
        }
...
int TrajOptimizer::optimize_traj_lmbm(const Eigen::Matrix3d &initS, const Eigen::Matrix3d &finalS, const Eigen::VectorXd &opt_x, const int N, Trajectory<TRAJ_ORDER> &traj) {
    total_opt_time = 0.0;
    total_sdf_time = 0.0;
    total_AABB_time = 0.0;
    ros::Time start_time = ros::Time::now();
    ...
    initState = initS;
    finalState = finalS;
    // calculate the coefficients of the trajectory
    minco.setConditions(initState, finalState, pieceN);
    const int total_opt_variable_num = temporalDim + spatialDim;
    //  malloc (short for memory allocate)
    //  sizeof(double) gives the size of one double in bytes.
    x_variable = (double *)malloc(sizeof(double) * total_opt_variable_num);
    // map opt_x to x_variable
