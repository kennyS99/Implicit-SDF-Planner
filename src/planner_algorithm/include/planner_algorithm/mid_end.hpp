...
public:
    ...
    template <typename EIGENVEC>
    static inline void backwardGradP(const Eigen::VectorXd &xi, const Eigen::Matrix3Xd &gradP, EIGENVEC &gradXi) {
    ...
    bool inline grad_cost_dir(
        const Eigen::Vector3d &pos, Eigen::Vector3d &gradp, double &cost_p,
        const int indexi)  // j=0-obj.integralRes
    {
        cost_p = 0.0;
        gradp.setZero();
        // waypoint xyz locations by A*
        Eigen::Vector3d ref_pos = ref_points.col(indexi);
        Eigen::Vector3d pos_diff = pos - ref_pos;
        // the cost = actual distance^3
        cost_p = pow(pos_diff.norm(), 3);
        gradp = 3 * pow(pos_diff.norm(), 2) * pos_diff.normalized();
        double cost = 0.0;
        cost += cost_p;
        // Return true if the cost is greater than 0, otherwise return false
        return (cost > 0);
    }

    static inline void addPosePenalty(void *ptr, const Eigen::VectorXd &T, const Eigen::MatrixX3d &coeffs, double &cost, Eigen::VectorXd &gradT, Eigen::MatrixX3d &gradC) {
        ...
        // make sure the constraints_count is equal to the number of wayPoints
        ...
        for (int i = 0; i < constraints_count; ++i) {
            segment = i + 1;
            // The block<Rows, Cols>(startRow, startCol) function in Eigen is used to extract a submatrix (block) from a larger matrix.
            ...
            if (obj.grad_cost_dir(pos, grad_tmp_p, cost_tmp_p, i)) {
                // update the partialGradByTimes and partialGradByCoeffs based on the pos_gradiant,
                // alpha, preset-weightPR
    ...
    static inline double costFunction(void *ptr, const Eigen::VectorXd &x, Eigen::VectorXd &g, double &p_cost) {
        // 优化变量的排列顺序为[tau, xi],tau为时间变量,xi为控制点变量。它们与轨迹之间的关系如下:
        // 轨迹由n段quintic polynomial(5 degree polynomial)组成,每段的时间间隔为tau[i]。
        ...
        double cost = 0;
        // solved polynomial coefficients can be used to define a curve between two points
        obj.minco.setParameters(obj.points, obj.times);

        // // Print the matrix b
        // const Eigen::MatrixX3d &b = obj.minco.getCoeffs();
        // std::cout << "Matrix b before partial:" << std::endl << b << std::endl;

        // cost
        obj.minco.getEnergy(cost);
        // find jacobian
        obj.minco.getEnergyPartialGradByCoeffs(obj.partialGradByCoeffs);  // ∂J/∂c
        obj.minco.getEnergyPartialGradByTimes(obj.partialGradByTimes);    // ∂J/∂T
        // new_variable = old_variable − α * gradient
        // where α is the learning rate.

        obj.minco.getTrajectory(obj.step_traj);

        addPosePenalty(ptr, obj.times, obj.minco.getCoeffs(), cost, obj.partialGradByTimes, obj.partialGradByCoeffs);
...
public:
    ...
    bool getOriTraj(
        const Eigen::Matrix3d initS, const Eigen::Matrix3d finalS, const std::vector<Eigen::Vector3d> &Q, Eigen::VectorXd T, std::vector<Eigen::Vector3d> acc_list,
        std::vector<Eigen::Matrix3d> rot_list, const int N, Trajectory<TRAJ_ORDER> &traj, Eigen::VectorXd &opt_x);
    ...
    static inline double costaltitude(const Eigen::Vector4d &wxyz_rotate, const Eigen::Matrix3d &rotate_ref) {
        ...
        double cost = 2 * a0 * (2 * y * y + 2 * z * z - 1) + 2 * b1 * (2 * x * x + 2 * z * z - 1) + 2 * c2 * (2 * x * x + 2 * y * y - 1) + 2 * a1 * (2 * w * z - 2 * x * y) -
            2 * a2 * (2 * w * y + 2 * x * z) - 2 * b0 * (2 * w * z + 2 * x * y) + 2 * b2 * (2 * w * x - 2 * y * z) + 2 * c0 * (2 * w * y - 2 * x * z) - 2 * c1 * (2 * w * x + y * z) + 6;
        return cost;
    }
    ...
    static inline void addTimeIntPenalty(void *ptr, const Eigen::VectorXd &T, const Eigen::MatrixX3d &coeffs, double &cost, Eigen::VectorXd &gradT, Eigen::MatrixX3d &gradC) {
        OriTraj &obj = *(OriTraj *)ptr;
    ...
    static inline void smoothMinabs(const double &x, const double &c, const double &mu, double &f, double &df) {
