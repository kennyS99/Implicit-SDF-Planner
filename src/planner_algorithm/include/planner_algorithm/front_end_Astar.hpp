    ...
    for (int i = -1; i < 2; i++) {
        for (int j = -1; j < 2; j++) {
            for (int k = -1; k < 2; k++) {
                ...
                t1 = ros::Time::now();
                // initial path search without map inflation
                cond = (environment->occupancy_map)->isIndexValid(vi) && !(environment->occupancy_map)->isIndexOccupiedFlate(vi, 0);
                t2 = ros::Time::now();
                /*check if the robot shape is collision with the environment

                */
                cond = cond && sv_manager->checkKernelValue(fr, fp, cr, cp, vi);

                if (fr != 0 && fp != 0 && cr != 0 && cp != 0) {
                    ROS_ERROR("fr: %f, fp: %f, cr: %f, cp: %f", fr, fp, cr, cp);
                }
...
inline void AstarPathSearcher::AstarPathSearch(Vector3d start, Vector3d end) {
    ...
    while (!openSet.empty()) {
        ...
        if (currentPtr->index == goalIdx) {
            ...
            success_flag = true;
            ROS_WARN(
                "[A*]{sucess}  Time in GetNeighbor is %f ms, kernel time is %f ms, "
                "about %f %",
                total_time,
                total_kernel_time,
                100 * total_kernel_time / total_time);
...
// generate SE3 path (translation + rotation)
