...
public:
    ...
    static inline void forwardP(const Eigen::VectorXd &xi, Eigen::Matrix3Xd &P) {
    ...
    static inline void forwardP(const double *xi, Eigen::Matrix3Xd &P, int Space_dim) {
    ...
    static inline void backwardP(const Eigen::Matrix3Xd &P, EIGENVEC &xi) {
    ...
    // tao--->T
    static inline void forwardT(const double *tau, Eigen::VectorXd &T, const int Time_dim) {
        T.resize(Time_dim);
        double temp;
        for (int i = 0; i < Time_dim; i++) {
            temp = *(tau + i);
            T(i) = temp > 0.0 ? ((0.5 * temp + 1.0) * temp + 1.0) : 1.0 / ((0.5 * temp - 1.0) * (temp) + 1.0);
        }
        return;
    }
    // T--->tao
    ...
    static inline void backwardGradT(const double *tau, const Eigen::VectorXd &gradT, double *gradTau, const int sizeTau) {
        double denSqrt;
    ...
    template <typename EIGENVEC>
    static inline void backwardGradP(const Eigen::Matrix3Xd &gradP, EIGENVEC &gradXi) {
    ...
    static inline void backwardGradP(const Eigen::Matrix3Xd &gradP, double *gradXi, const int Space_dim) {
        for (int i = 0; i < Space_dim; ++i) {
    ...
    static inline double costFunctionLmbm(void *ptr, const double *x_variable, double *g, const int n) {
        ...
        addSaftyPenaOnSweptVolumeParallel(ptr, obj.times, obj.minco.getCoeffs(), cost, obj.partialGradByTimes, obj.partialGradByCoeffs);
        ...
        addTimeIntPenaltyParallel(ptr, obj.times, obj.minco.getCoeffs(), cost, pos_cost_tmp, obj.partialGradByTimes, obj.partialGradByCoeffs);
        ...
        obj.minco.propogateGrad(
            obj.partialGradByCoeffs,
            obj.partialGradByTimes,  // ∂Cost/∂c,T -> ∂Cost/∂q,T
    ...
    static inline void addTimeIntPenaltyParallel(void *ptr, const Eigen::VectorXd &T, const Eigen::MatrixX3d &coeffs, double &cost, double &pos_cost, Eigen::VectorXd &gradT, Eigen::MatrixX3d &gradC) {
        TrajOptimizer &obj = *(TrajOptimizer *)ptr;
...
#pragma omp critical
            {
                gradC.block<6, 3>(i * 6, 0) +=
                    (beta0 * gradPosTotal.transpose() + beta1 * gradVelTotal.transpose() + beta2 * gradAccTotal.transpose() + beta3 * gradJerTotal.transpose()) * node * step;
                gradT(i) += (gradPosTotal.dot(vel) + gradVelTotal.dot(acc) + gradAccTotal.dot(jer) + gradJerTotal.dot(sna)) * alpha * node * step + node * integralFrac * pena;
                cost += node * step * pena;
            }
        }
        return;
    ...
    static inline void addSaftyPenaOnSweptVolumeParallel(
        void *ptr, const Eigen::VectorXd &T, const Eigen::MatrixX3d &coeffs, double &cost, Eigen::VectorXd &gradT, Eigen::MatrixX3d &gradC /**,bool output=false*/) {
        TrajOptimizer &obj = *(TrajOptimizer *)ptr;
        // config preload coeff
        const double weightPos = obj.weight_p;
        const double neighbor_dis = 2 * obj.conf.occupancy_resolution;
        const int pieceNum = T.size();
        // check very point cloud in AABB ranged
#pragma omp parallel for num_threads(obj.threads_num) schedule(dynamic)
        for (int pointindex = 0; pointindex < obj.parallel_points_num; ++pointindex) {
            Eigen::Vector3d gradp_rel;
            // occupied grid position in the waypoint neighborhood
            ...
            if (obj.grad_cost_p_sw(pos_eva, pos, rotate, quat, sdf_value, gradp_eva, gradp_rel, violaPosPenaGrad, violaQuatPenaGrad, violaPosPena)) {
                gradPos += weightPos * violaPosPenaGrad;
                gradQuat += weightPos * violaQuatPenaGrad;
                pena += weightPos * violaPosPena;
            }
            obj.flatmap.backwardthreadsafe(vel, acc, jer, gradPos, gradVel, gradQuat, gradOmg, gradPosTotal, gradVelTotal, gradAccTotal, gradJerTotal);
            Eigen::Matrix<double, 6, 3> gdC = (beta0 * gradPosTotal.transpose() + beta1 * gradVelTotal.transpose() + beta2 * gradAccTotal.transpose() + beta3 * gradJerTotal.transpose());

            double gdT = (-gradPosTotal.dot(vel) + -gradVelTotal.dot(acc) + -gradAccTotal.dot(jer) + -gradJerTotal.dot(sna));
...
public:
    ...
    void setParam(ros::NodeHandle &nh, Config &config) {
        ...
        // flatmap;
        flatmap.reset(config.vehicleMass, config.gravAcc, config.horizDrag, config.vertDrag, config.parasDrag, config.speedEps);
    ...
    bool optimize_traj(const Eigen::Matrix3d &initS, const Eigen::Matrix3d &finalS, const Eigen::VectorXd &opt_x, const int N, Trajectory<TRAJ_ORDER> &traj);

    int optimize_traj_lmbm(const Eigen::Matrix3d &initS, const Eigen::Matrix3d &finalS, const std::vector<Eigen::Vector3d> &Q, const Eigen::VectorXd &T, const int N, Trajectory<TRAJ_ORDER> &traj);

    int optimize_traj_lmbm(const Eigen::Matrix3d &initS, const Eigen::Matrix3d &finalS, const Eigen::VectorXd &opt_x, const int N, Trajectory<TRAJ_ORDER> &traj);
    ...
    bool inline grad_cost_p(const Eigen::Vector3d &pos, const Eigen::Matrix3d &R, const Eigen::Vector4d &quat, Eigen::Vector3d &gradp, Eigen::Vector4d &grad_quat, double &costp) {
        costp = 0.0;
        gradp.setZero();
        grad_quat.setZero();
    ...
    bool inline grad_cost_p_sw(
        const Eigen::Vector3d &pos_eva, const Eigen::Vector3d &pos_obj, const Eigen::Matrix3d &rotate, const Eigen::Vector4d &quat, const double sdf_value, const Eigen::Vector3d &gradp_eva,
        Eigen::Vector3d &gradp_rel, Eigen::Vector3d &gradp, Eigen::Vector4d &grad_quat, double &costp) {
    ...
    static inline int earlyexitLmbm(void *instance, const double *x, const int k) {
    ...
    static inline int earlyExit(void *instance, const Eigen::VectorXd &x, const Eigen::VectorXd &g, const double fx, const double step, const int k, const int ls) {
        TrajOptimizer &obj = *(TrajOptimizer *)instance;
        obj.iter++;
