...
class PCSmapManager {
public:
    vector<Eigen::Vector3d> AABB_visited_points;
    Eigen::Vector3d AAbb_center_point;
    ...
    /**
     * @brief Creates a padded and bit-compressed representation of a 3D occupancy grid map.
     *
     * Padding: If an algorithm needs to check a 3x3x3 neighborhood around each cell, padding ensures that cells near the edges have a full neighborhood to consider, preventing out-of-bounds errors.
     *
     * Bit-Compressed: Compressing the map into a bitwise format reduces the amount of memory required to store the map.
     *
     * @pre The occupancy map must be initialized before calling this function.
     * @param kernel_size The size of the kernel.
     * @return A pointer to the bit-compressed and padded occupancy map.
     */
    inline uint8_t* generateMapKernel(const int& kernel_size) {
        // kernel_size =15
        int side_size = (kernel_size - 1) / 2;
        int kernel_x_size = occupancy_map->X_size + 2 * side_size;
        int kernel_y_size = occupancy_map->Y_size + 2 * side_size;
        int kernel_z_size = occupancy_map->Z_size + 2 * side_size;
        // 1 byte = 8 bits, this ensure enough bytes to store z size
        int bytes_len_of_last_dim = (kernel_z_size + 7) / 8;
        int size_yz = kernel_y_size * bytes_len_of_last_dim;
        int bytes_len = kernel_x_size * kernel_y_size * bytes_len_of_last_dim;
    ...
    inline void projInMap(Eigen::Vector3d& pt) const {
        if (pt(0) < boundary_xyzmin(0)) {
            pt(0) = boundary_xyzmin(0);
        }
        if (pt(1) < boundary_xyzmin(1)) {
            pt(1) = boundary_xyzmin(1);
        }
        if (pt(2) < boundary_xyzmin(2)) {
            pt(2) = boundary_xyzmin(2);
        }
        if (pt(0) > boundary_xyzmax(0)) {
            pt(0) = boundary_xyzmax(0);
        }
        if (pt(1) > boundary_xyzmax(1)) {
            pt(1) = boundary_xyzmax(1);
        }
        if (pt(2) > boundary_xyzmax(2)) {
            pt(2) = boundary_xyzmax(2);
        }
    }
     ...
     * @param points 引用格式，用于接收结果
     * halfbdx = halfbdy = halfbdz = 5
     */
    inline void getPointsInAABBOutOfLastOne(
        const Eigen::Vector3d& center, const Eigen::Vector3d& center_last, double halfbdx, double halfbdy, double halfbdz, const Eigen::Vector3d offset = Eigen::Vector3d::Zero()) {
        AABB_visited_points.clear();
        Eigen::Vector3d AABB_single_point;

        // generate a rectangle bounding box
        ...
        const Eigen::Vector3i idcorner1 = occupancy_map->getGridIndex(corner1);
        const Eigen::Vector3i idcorner2 = occupancy_map->getGridIndex(corner2);
        cout << "Bottom-Left-Front (BLF) edge: " << idcorner1.transpose() << endl;
        cout << "Top-Right-Back (TRB) edge: " << idcorner2.transpose() << endl;
        // generate a rectangle bounding box for last way point
        ...
        cout << "Bottom-Left-Front (BLF) edge last: " << idcorner1_l.transpose() << endl;
        cout << "Top-Right-Back (TRB) edge last: " << idcorner2_l.transpose() << endl;
        // check every grid in the AABB box
        for (int i = idcorner1(0); i <= idcorner2(0); i++) {
            for (int j = idcorner1(1); j <= idcorner2(1); j++) {
                for (int k = idcorner1(2); k <= idcorner2(2); k++) {
                    // if current grid index is not in last AABB box
                    if (i > idcorner2_l(0) || i < idcorner1_l(0) || j > idcorner2_l(1) || j < idcorner1_l(1) || k > idcorner2_l(2) || k < idcorner1_l(2)) {
                        // if the grid is occupied
                        AABB_single_point[0] = i;
                        AABB_single_point[1] = j;
                        AABB_single_point[2] = k;
                        AABB_visited_points.push_back(AABB_single_point);
                        // cout << "i: " << i << " j: " << j << " k: " << k << endl;
                        // cout << "is index occupied?" << occupancy_map->isIndexOccupied(i, j, k) << endl;
                        // cout << "unifiedID:" << unifiedID(i, j, k) << endl;

                        if (occupancy_map->isIndexOccupied(i, j, k)) {
                            // convert the grid index to the center point of the grid
                            AAbb_center_point = occupancy_map->getGridCubeCenter(i, j, k);
