...
bool GridMap3D::isIndexOccupied(int ix, int iy, int iz) {
    ...
    return grid_map[toAddr(ix, iy, iz)] != 0;
}
...
void GridMap3D::generateESDF3d() {
    ...
    for (int x = min_esdf(0); x <= max_esdf(0); x++) {
        for (int y = min_esdf(1); y <= max_esdf(1); y++) {
            fillESDF(
                [&](int z) { return isIndexOccupied(x, y, z) == 1 ? 0 : std::numeric_limits<double>::max(); },
                [&](int z, double val) { grid_esdf_buffer1[toAddr(x, y, z)] = val; },
                min_esdf(2),
                max_esdf(2),
                2);
        }
    ...
    for (int x = min_esdf(0); x <= max_esdf(0); x++) {
        for (int z = min_esdf(2); z <= max_esdf(2); z++) {
            fillESDF([&](int y) { return grid_esdf_buffer1[toAddr(x, y, z)]; }, [&](int y, double val) { grid_esdf_buffer2[toAddr(x, y, z)] = val; }, min_esdf(1), max_esdf(1), 1);
        }
    }

    for (int y = min_esdf(1); y <= max_esdf(1); y++) {
        for (int z = min_esdf(2); z <= max_esdf(2); z++) {
            fillESDF([&](int x) { return grid_esdf_buffer2[toAddr(x, y, z)]; }, [&](int x, double val) { grid_esdf[toAddr(x, y, z)] = grid_resolution * std::sqrt(val); }, min_esdf(0), max_esdf(0), 0);
        }
    }
    ...
    for (int x = min_esdf(0); x <= max_esdf(0); x++) {
        for (int y = min_esdf(1); y <= max_esdf(1); y++) {
            fillESDF(
                [&](int z) { return grid_map_buffer_neg[toAddr(x, y, z)] == 1 ? 0 : std::numeric_limits<double>::max(); },
                [&](int z, double val) { grid_esdf_buffer1[toAddr(x, y, z)] = val; },
                min_esdf(2),
                max_esdf(2),
                2);
    ...
    for (int y = min_esdf(1); y <= max_esdf(1); y++) {
        for (int z = min_esdf(2); z <= max_esdf(2); z++) {
            fillESDF(
                [&](int x) { return grid_esdf_buffer2[toAddr(x, y, z)]; },
                [&](int x, double val) { grid_map_buffer_neg[toAddr(x, y, z)] = grid_resolution * std::sqrt(val); },
                min_esdf(0),
                max_esdf(0),
                0);
        }
