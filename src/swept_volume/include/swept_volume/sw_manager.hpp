...
public:
    std::map<std::string, std::function<Generalshape *(const Config &)>> shapeConstructors = {
        {"CSG",
         [](const Config &conf) {
             std::cout << "\033[35m=======CSG======= "
                       << "\033[0m" << std::endl;
             return new CSG(conf);
         }},
        {"Torus",
         [](const Config &conf) {
             std::cout << "\033[35m=======Torus======= "
                       << "\033[0m" << std::endl;
             return new Torus(conf);
         }},
        {"Torus_big",
         [](const Config &conf) {
             std::cout << "\033[35m=======Torus_big======= "
                       << "\033[0m" << std::endl;
             return new Torus_big(conf);
         }},
        {"Cappedtorus",
         [](const Config &conf) {
             std::cout << "\033[35m=======Cappedtorus======= "
                       << "\033[0m" << std::endl;
             return new Cappedtorus(conf);
         }},
        {"Trefoil",
         [](const Config &conf) {
             std::cout << "\033[35m=======Trefoil======= "
                       << "\033[0m" << std::endl;
             return new Trefoil(conf);
         }},
        {"Table",
         [](const Config &conf) {
             std::cout << "\033[35m=======Table======= "
                       << "\033[0m" << std::endl;
             return new Table(conf);
         }},
        {"CappedCone",
         [](const Config &conf) {
             std::cout << "\033[35m=======CappedCone======= "
                       << "\033[0m" << std::endl;
             return new CappedCone(conf);
         }},
        {"RoundedCone",
         [](const Config &conf) {
             std::cout << "\033[35m=======RoundedCone======= "
                       << "\033[0m" << std::endl;
             return new RoundedCone(conf);
         }},
        {"WireframeBox",
         [](const Config &conf) {
             std::cout << "\033[35m=======WireframeBox======= "
                       << "\033[0m" << std::endl;
             return new WireframeBox(conf);
         }},
        {"BendLinear",
         [](const Config &conf) {
             std::cout << "\033[35m=======BendLinear======= "
                       << "\033[0m" << std::endl;
             return new BendLinear(conf);
         }},
        {"BendLinear_big",
         [](const Config &conf) {
             std::cout << "\033[35m=======BendLinear_big======= "
                       << "\033[0m" << std::endl;
             return new BendLinear_big(conf);
         }},
        {"TwistBox",
         [](const Config &conf) {
             std::cout << "\033[35m=======TwistBox======= "
                       << "\033[0m" << std::endl;
             return new TwistBox(conf);
         }},
        {"BendBox",
         [](const Config &conf) {
             std::cout << "\033[35m=======BendBox======= "
                       << "\033[0m" << std::endl;
             return new BendBox(conf);
         }},
        {"SmoothDifference",
         [](const Config &conf) {
             std::cout << "\033[35m=======SmoothDifference======= "
                       << "\033[0m" << std::endl;
             return new SmoothDifference(conf);
         }},
        {"SmoothIntersection",
         [](const Config &conf) {
             std::cout << "\033[35m=======SmoothIntersection======= "
                       << "\033[0m" << std::endl;
             return new SmoothIntersection(conf);
         }},
        {"SmoothIntersection_big", [](const Config &conf) {
             std::cout << "\033[35m=======SmoothIntersection_big======= "
                       << "\033[0m" << std::endl;
...
public:
    ...
    inline void init(ros::NodeHandle &nh, Config &conf) {
        ...
        flatness.reset(conf.vehicleMass, conf.gravAcc, conf.horizDrag, conf.vertDrag, conf.parasDrag, conf.speedEps);
    ...
    inline Eigen::MatrixXd getRobotShapeMesh() { return current_robot_shape->mesh; }
     ...
     */
    inline void getStateOnTrajStamp(const double &time_stamp, Eigen::Vector3d &xt, Eigen::Vector3d &vt, Eigen::Matrix3d &Rt, Eigen::Matrix3d &VRt) {
        ...
        w_hat << 0, -omg(2), omg(1), omg(2), 0, -omg(0), -omg(1), omg(0), 0;
        VRt = Rt * w_hat;
    }
     ...
     * @param Rt 引用格式，获得time_stamp时刻的姿态
     */
    inline void getStateOnTrajStamp(const double &time_stamp, Eigen::Vector3d &xt, Eigen::Matrix3d &Rt) {
     ...
     */
    inline Eigen::Vector3d posEva2Rel(const Eigen::Vector3d &pos_eva, const Eigen::Vector3d &xt, const Eigen::Matrix3d &Rt) { return (Rt.transpose() * (pos_eva - xt)); }

    /**
     * 在轨迹上离散采样，初步获取时间种子
     * 首先将轨迹粗糙离散（0.2）， 找到所有小于安全距离的范围轨迹，然后在范围轨迹内上再次离散（0.02），找到最近的点，并保留这个轨迹点所对应的时刻到range_ts
     * 如果一段轨迹找到多个segments小于安全范围，那么在这多个segments之间再次离散（0.02），找到最近的点
     * @param pos_eva 世界系下需要查询的点(AABB box范围内的占据grid)
     * @param dt 筛选种子点的分辨率 0.02
     * @return 找到的时间种子
     */
    inline double choiceTInit(const Eigen::Vector3d &pos_eva, const double dt, vector<double> &range_l, vector<double> &range_r, vector<double> &range_ts, double &mini_distance) {
        double rough_dt = 0.2;
        ...
        Eigen::Vector3d xt, vt;
        Eigen::Matrix3d Rt;
        // traj_duration is the total duration of the trajectory)
        // Loop over the trajectory duration in steps of rough_dt
        for (double t = 0; t < traj_duration; t += rough_dt) {
            // get the state of the robot at time t, xt is the position, Rt is the rotation matrix
            getStateOnTrajStamp(t, xt, Rt);
            // Find the ESDF value of the robot surface closest to the a single query point
            dis = current_robot_shape->getonlySDF(posEva2Rel(pos_eva, xt, Rt));
            // mindis = 10^9 initially, so the first time it will be updated
            if (dis < mindis) {
                // update the current time and the minimum distance
                time_seed = t;
                mindis = dis;
            }
            // If the distance is less than the threshold, when the first time dis < threshold,update the lower (current time - rough_dt) and upper (current time) bounds, then the second time
            // dis > threshold, update the upper bound (current time + rough_dt), keep the lower bound of time.
            // if next following time sample, distance is greater than threshold, update the upper bound (current time + rough_dt), and store the lower and upper bounds in the range_l and range_r
            // vectors this process will repeat when the distance in next time sample is less than the threshold
        ...
        // in the last loop extract rough range of time in trajecotry, find the closet distance point to the query point, and store the correspoding time in trajectory into range_ts
        // the query point is extract from the AABB box of the occupied grid in each wayPoint
        int N = range_l.size();
     ...
     * @return SDF值
     */
    inline double getSDFWithGradWhenRobotAtState(const Eigen::Vector3d &pos_eva, Eigen::Vector3d &grad_prel, const Eigen::Vector3d &xt, const Eigen::Matrix3d &Rt) {
    ...
    inline double getSDFofSweptVolume(const Eigen::Vector3d &pos_eva, double &time_seed_f, Eigen::Vector3d &grad_prel, bool set_ts) {
        ...
        if (set_ts == false) {
            ts = choiceTInit(pos_eva, dtime, range_l, range_r, range_ts, mini_distance);
        }

        int range_count = range_l.size();
        for (int i = 0; i < range_count; i++) {
            double temp_ts = range_ts[i];
            // make sure the select time range is inside the trajectory total time
            double tmin_ = std::max(0.0, range_l[i]);
            double tmax_ = std::min(range_r[i], traj_duration);
            // sdf_star is the smallest sdf value in this time stamp range
            // t_star is the corresponding time stamp
            // 通过梯度下降法，找到最小的sdf_star和对应的t_star
            // 该方法先将通过choiceTInit函数找到的range_l, range_r当作tmin_, tmax_的范围，然后在该范围内先将choiceTInit找到的离pos_eva最近的轨迹时刻当作temp_ts，然后以temp_ts为初值计算当时的sdf值以及对应的梯度，将temp_ts按照下降梯度的方向更新10次，且每次更新step size*0.5,将此时最小的sdf_star和对应的t_star作为最终结果返回
            ...
            if (sdf_star < min_sdf_star) {
                ...
                time_seed_f = t_star;
                if (need_grad_prel) {
                    // get the gradient of robot correspoding to the pos_eva @ the time stamp t_star
                    grad_prel = getGradPrelAtTimeStamp(pos_eva, t_star);
                }
            }
        }
     ...
     * @param traj_stamp 时刻
     */
    inline void setTrajStamp(const double &traj_stamp) { trajStamp = traj_stamp; }
     ...
     */
    inline bool isTrajCollide() { return false; }
    ...
    // check the index of occupancy grid is collide with the robot shape
    // if collide return false, else return true
    inline bool kernelConv(int kernel_i, int kernel_j, const Eigen::Vector3i &ind) {
        ...
        if (useByteKernel == false) {
            ...
            // bytes_len_of_last_dim = 2
            int bytes_len_of_last_dim = (kernel_size + 7) / 8;
            // map_Xsize is the occupancy map X size
    ...
    inline bool visit_kernels_by_distance(int &returni, int &returnj, int start_x, int start_y, const Eigen::Vector3i &ind, int maxdeepth = 800) {
        ...
        // initialize the entire x and y kernel map as unvisited
        ...
        // start_x, start_y is the farther node roll and pitch
        ...
        while (!q.empty()) {
            ...
            q.pop();
            // check farther node of occupancy map index if in the center of the kernel
            ...
            // along the x and y axis direction check
    ...
    inline bool checkKernelValue(double father_roll, double father_pitch, double &child_roll, double &child_pitch, const Eigen::Vector3i &ind) {
        ...
        bool res = false;
        // BFS
        // ind is neighbor point index of farther node
        // father_i = 5, father_j = 5
    ...
    inline void process(const Trajectory<TRAJ_ORDER> &traj) {
        ...
        if (init < 200) {
            ...
            vis->visABoxWithId(
                Eigen::Vector3d(config.offsetAABBbox[0], config.offsetAABBbox[1], config.offsetAABBbox[2]), Eigen::Vector3d(2 * bdx / 3.0, 2 * bdy / 3.0, 2 * bdz / 3.0), "AABBbox", vis::yellow, 0.4,
                2);
            init++;
        }
        ...
        if (delta > 0.0 && delta < traj.getTotalDuration()) {
            ...
            Eigen::Vector4d quat;
            Eigen::Vector3d omg;
            flatness.forward(traj.getVel(delta), traj.getAcc(delta), traj.getJer(delta), 0.0, 0.0, thr, quat, omg);
    ...
    /**
     * This function implements a gradient descent optimization algorithm to find the time stamp (x) that minimizes the Signed Distance Function (SDF) value (fx) for a given evaluation point (pos_eva)
     * within a specified time range (t_min to t_max). finally, update the smallest (SDF) value (fx) and the corresponding time stamp (x). (SweptVolumeManager类专用)
     * @param momentum 动量//useless
     * @param t_min    搜索范围下界
     * @param t_max    搜索范围上界
     * @param x0       之前的方法找到距离pos_eva最近的点的轨迹对应t
     * @param fx       引用格式，接收SDF* update value
     * @param x        引用格式，接收t* update value
     * @param pos_eva  评价点的坐标
     */
    inline void gradientDescent(double momentum, double t_min, double t_max, const double x0, double &fx, double &x, const Eigen::Vector3d &pos_eva) {
        ...
        while (iter < max_iter && !stop && abs(x - prev_x) > tol) {
            ...
            for (int div = 1; div < 10; div++) {
                iter = iter + 1;
                // x is the current trajectory time
                projection = x;
                g = getSDF_DOTAtTimeStampOptimized(pos_eva, projection);
                //  adjusting x towards the direction that decreases fx
                x_candidate = x - tau * ((int)(g > 0) - (int)(g < 0));
                // make sure the x_candidate is in the range of [t_min, t_max]
                ...
                // if new SDF value is smaller than the previous one, update x and fx
    ...
    void gradient_descent(
        double momentum, double t_min, double t_max, const double x0, double &fx, double &x, const Eigen::Vector3d &pos_eva, std::vector<double> &intervals, std::vector<double> &values,
    ...
    std::function<double(const Eigen::RowVector3d &, double &, std::vector<std::vector<double>> &, std::vector<std::vector<double>> &, std::vector<std::vector<double>> &)> scalarFunc =
        [&](const Eigen::RowVector3d &P, double &time_seed, std::vector<std::vector<double>> &intervals, std::vector<std::vector<double>> &values, std::vector<std::vector<double>> &minima) -> double {
