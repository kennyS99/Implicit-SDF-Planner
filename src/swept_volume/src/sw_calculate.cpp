...
void sw_calculate::calculation(
    const Eigen::RowVector3d &p0,
    const std::vector<Eigen::RowVector3i> &init_voxels,
    const std::vector<double> &t0,
    const std::function<double(const Eigen::RowVector3d &, double &, std::vector<std::vector<double>> &, std::vector<std::vector<double>> &, std::vector<std::vector<double>> &)> scalarFunc,
    const double eps,
    ...
    while (queue.size() > 0) {
        ...
        std::array<Eigen::RowVector3d, 8> cubeCorners = {
            ctr + half_eps * (bx + by + bz).cast<double>(),
            ctr + half_eps * (bx + by - bz).cast<double>(),
            ctr + half_eps * (-bx + by - bz).cast<double>(),
            ctr + half_eps * (-bx + by + bz).cast<double>(),
            ctr + half_eps * (bx - by + bz).cast<double>(),
            ...
            if (nbr != visited.end()) {
                ...
                for (int i = 0; i < 4; i++) {
                    if (zvv[n][i] != -1) {
                        ...
                        if ((CS_vector[cube[zvv[n][i]]] > cubeScalars[zvv[n][i]] && (CS_vector[cube[zvv[n][i]]] * cubeScalars[zvv[n][i]]) < 0) ||
                            CS_vector[cube[zvv[n][i]]] > (cubeScalars[zvv[n][i]] + 1e-3)) {
                            if (!flag) {
