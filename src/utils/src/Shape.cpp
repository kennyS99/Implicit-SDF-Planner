...
inline double Generalshape::getonlySDF(const Eigen::RowVector3d &pos_rel) {
    ...
    // This step determines if the point is inside or
    // outside the shape by using the fast winding number algorithm.
    igl::fast_winding_number(fwn_bvh, 2.0, pos_rel, w);
    // s is signed +-
    double s = 1.0 - 2.0 * w(0);
...
inline double Generalshape::getonlySDF(const Eigen::RowVector3d &pos, const Eigen::Matrix3d &R_obj) {
    int i;
    // store point on the surface of the 3D mesh that is nearest to a given point in space
    Eigen::RowVector3d c;
    Eigen::VectorXd w{Eigen::VectorXd::Zero(1)};
...
inline Eigen::Vector3d Generalshape::getonlyGrad1(const Eigen::RowVector3d &pos_rel) {
    int i;
    // store the postion of point on the surface of the 3D mesh that is nearest to a given point in space
    Eigen::RowVector3d c;
    Eigen::VectorXd w{Eigen::VectorXd::Zero(1)};
    // w is the winding number used to determine if the point is inside or outside the shape
    igl::fast_winding_number(fwn_bvh, 2.0, pos_rel, w);
    // Determines the sign s of the SDF value. If the winding number w(0) is 0.5, s will be positive (indicating pos_rel is inside), and if w(0) is -0.5, s will be negative (indicating pos_rel is
    // outside).
    double s = 1. - 2. * w(0);
    tree.squared_distance(V, F, pos_rel, i, c);
    // c - pos_rel gives the vector from the query point pos_rel to the closest point c.
    // The negation -(c - pos_rel) gives the vector pointing from pos_rel to c.
