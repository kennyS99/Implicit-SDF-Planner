 ...
 */
typedef double (*lbfgs_evaluate_t)(void *instance, const Eigen::VectorXd &x, Eigen::VectorXd &g, double &p_cost);
 ...
 */
typedef int (*lbfgs_progress_t)(void *instance, const Eigen::VectorXd &x, const Eigen::VectorXd &g, const double fx, const double step, const int k, const int ls);
...
inline int line_search_lewisoverton(
    Eigen::VectorXd &x, double &f, double &fpos, Eigen::VectorXd &g, double &stp, const Eigen::VectorXd &s, const Eigen::VectorXd &xp, const Eigen::VectorXd &gp, const double stpmin,
    ...
    while (true) {
        ...
        // }
        double stp_conv1 = (1.0 - stp) > 0.0 ? (0.05 * (1.0 - stp)) : 0.0;
        ...
        else if (f > finit + stp * dgtest + tol_sw * (stp_conv1 * finit)) {
            nu = stp;
            brackt = true;
            // cond1 ++;
        } else {
...
inline int lbfgs_optimize(
    Eigen::VectorXd &x, double &f, lbfgs_evaluate_t proc_evaluate, lbfgs_stepbound_t proc_stepbound, lbfgs_progress_t proc_progress, void *instance, const lbfgs_parameter_t &param) {
    ...
    // printf("------------2------------");
    cout << "x value before costFunction" << x.transpose() << endl;
    cout << "Evaluate the function value and its gradient" << endl;
    fx = cd.proc_evaluate(cd.instance, x, g, p_cost);
    // printf("------------3------------");
    /* Store the initial value of the cost function. */
    cout << "Store the initial value of the cost function." << endl;
    pf(0) = fx;
    ...
    if (gnorm_inf / std::max(1.0, xnorm_inf) < param.g_epsilon) {
        ...
        */
        ...
        while (true) {
            ...
            ls = line_search_lewisoverton(x, fx, p_cost, g, step, d, xp, gp, step_min, step_max, cd, param);  // assert(d.dot(gp)<0)
            //    step step太小导致d*step与xp数量级差距太大，再减去xp有问题
            // {
            // std::cout<<"================================================="<<std::endl;
            // std::cout<<"=====d norm======"<<d.norm()<<std::endl;
            // std::cout<<"step:"<<step<<std::endl;
            // std::cout<<"=====delta x======"<<(d*step).transpose()<<std::endl;
            // std::cout<<"=====delta x plus xp======"<<(d*step+xp).transpose()<<std::endl;
            // std::cout<<"=====delta x plus xp minus x======"<<(d*step+xp-x).transpose()<<std::endl;
            // std::cout<<"g:  norm"<<g.norm()<<" "<<g.head(10).transpose()<<std::endl;
            // std::cout<<"gp:   norm"<<gp.norm()<<" "<<gp.head(10).transpose()<<std::endl;
            // std::cout<<"x:  norm"<<x.norm()<<" "<<x.head(10).transpose()<<std::endl;
            // std::cout<<"xp:   norm"<<xp.norm()<<" "<<xp.head(10).transpose()<<std::endl;
            ...
            */
            // std::cout<<"lm_s:"<<lm_s.transpose()<<std::endl;
            ys = lm_y.col(end).dot(lm_s.col(end));
            yy = lm_y.col(end).squaredNorm();  // yy等于0的情况下也有shit
            // std::cout<<"\033[35m end:"<<end<<" \033[0m     ys:"<<ys<<"   lm_y.col(end) norm:"<<lm_y.col(end).norm()<<"    lm_s.col(end) norm:"<<lm_s.col(end).norm()<<std::endl;
            // std::cout<<"\033[36m before lm_ys:\033[0m"<<lm_ys.transpose()<<std::endl;
            if (ys == 0) {
                std::cout << "\033[35m SHIT SHIT SHIT SHIT SHIT SHIT "
                          << "\033[0m" << std::endl;
                // abort();
            }
            // ys=(ys==0)?lm_ys.mean():ys;//避免步长太小除法0情况,打补丁
            ...
            */
            ...
            // std::cout<<"\033[36m after lm_ys:\033[0m"<<lm_ys.transpose()<<std::endl;
            if (ys > cau) {
                /*
                ...
                */
                ...
                for (i = 0; i < bound; ++i) {
                    ...
                    // std::cout<<""
                    // std::cout<<"alpha"<<" "<<lm_alpha(j)<<" with j:"<<j<<std::endl;
                ...
                // std::cout<<"lm_alpha:"<<lm_alpha.transpose()<<std::endl;
                // std::cout<<"d: "<<d.transpose()<<std::endl;
                // std::cout<<"\033[36m  lm_ys:\033[0m"<<lm_ys.transpose()<<std::endl;
                // std::cout<<"\033[36m  lm_y:\033[0m"<<lm_y.transpose()<<std::endl;
                for (i = 0; i < bound; ++i) {
                    ...
                    // std::cout<<"delta2 "<<"i:"<<i<<" beta:"<<beta<<" ----"<<(lm_alpha(j) - beta) * lm_s.col(j).transpose().head(4)<<std::endl;
                    j = (j + 1) % m; /* if (++j == m) j = 0; */
                }
            ...
            } else  // 打补丁
            {
                ...
                d *= olddnorm;  // 重新求取方向
                // end=0;//重新置换为初始条件？
                std::cout << "\033[35m there "
                          << "\033[0m" << std::endl;
                // std::cout<<"\033[35m SHIT SHIT SHIT there "<<"\033[0m"<<std::endl;

                // ++bound;//补充条件，原先有shit?
                // bound = m < bound ? m : bound;//补充条件，原先有shit?
                end = (end + 1) % m;  // 不应该增加？end取0,1,2....m-1//防止 lm_alpha(j) = lm_s.col(j).dot(d) / lm_ys(j);除0操作
            }
            std::cout << "\033[36m with norm:" << d.norm() << "\033[0m"
                      << "with step:" << step << std::endl;
            // std::cout<<"------------------------------------------------------------------------"<<std::endl;
            if (isinf(d.norm()) || isnan(d.norm())) {
                return LBFGSERR_INVALID_FUNCVAL;
                // abort();
            }
            olddnorm = d.norm();  // 打补丁
